...
def _load_rates_expectations():
    ...
    def _bounds(lbl):
        s = str(lbl).strip().replace("–", "-").replace("—", "-").replace("to", "-")
        s = s.strip("()[]{} ")
        parts = s.split("-")
        if len(parts) != 2:
...
# -------- KPIs
def _fmt_money_m(x):
    return "—M" if pd.isna(x) else f"{x:,.0f}M"
...
with col4: kpi_card("Fed Direction (mode)", f"{(k4_dir or '—')} ({int(round((0 if pd.isna(k4_p) else k4_p)*100))}%)")
